...
public class DoorTrapWaterCannon : MonoBehaviour
{
    ...
    public void On()
    {
        ...
        if (isOnProcessing)
        {
            ...
            AudioManager.instance.PlayTestBeep("물 대포 트랩이 이미 실행 중입니다.", waterMainParticle.gameObject.transform);
            return;
        }
        ...
        if (count <= 0)
        {
            ...
            AudioManager.instance.PlayTestBeep("물 대포 트랩 발사 횟수를 초과했습니다.", waterMainParticle.gameObject.transform);
            return;
        }
        ...
        AudioManager.instance.PlayTestBeep("강력한 물 대포 소리. 고압으로 분사되는 물줄기 소리. 촤아아악 하는 물 분사음과 튕기는 물 소리. 꺼질때까지 진행되어야함." , waterMainParticle.gameObject.transform);
