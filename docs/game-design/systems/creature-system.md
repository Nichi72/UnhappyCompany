...
- **특수 메커니즘**:
  ...
  - **기력 시스템**: 도망 시 기력 소모, 배회 시 회복
  - **지침 상태**: 기력 소진 시 도망 불가 (처치 용이)
  - **점액 배출**: 10초마다 이동속도 감소 디버프 지역 생성
  - **NavMesh 기반**: 안정적인 도망 경로 계산
- **대응 전략**: 
  - **🎯 스니킹 (쉬움)**: 뒤에서 조용히 걸어가서 백어택 - 가장 효율적
  - **🔫 원거리 (쉬움)**: 총으로 안전하게 처치 - 총알 소모하지만 확실함
  - **🏃 추격전 (시간 소모)**: 계속 쫓아가서 기력 소진시키기 - 시간 걸리지만 확실함
...
### 즉시 진행 (이번 주)
1. ✅ **Moo 크리처 기본 설계** 완료
2. 🔄 **Moo NavMesh 기반 이동 시스템** 구현
3. 🔄 **기력 시스템 코어 로직** 개발
4. **기계형 자원 밸런싱** (렘페이지 + RSP)

### 단기 목표 (다음 주)
1. 🔄 **Moo 시야/청각 감지 시스템** 완성
2. 🔄 **기력 UI 및 시각적 피드백** 추가
3. **3크리처 동시 테스트** 진행
4. **오후 페이즈 밸런싱** 1차 조정

### 중기 목표 (한 달 내)
1. **Moo 밸런싱 및 폴리싱** 완료
2. **모든 크리처 완성** 및 통합 테스트
3. **크리처별 학습 시스템** 구현
4. **4번째 크리처 (인간형)** 기획 시작
